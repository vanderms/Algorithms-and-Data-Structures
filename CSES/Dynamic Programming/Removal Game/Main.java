...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i < n; i++){
            int num = in.nextInt();
            prefix[i] = i == 0 ? num : num + prefix[i - 1];
            memo[i][i] = num;
        }
        ...
        int step = 1;
        while(step < n){
            long sum = start > 0 ? prefix[end] - prefix[start - 1] : prefix[end];
            memo[start][end] = sum - Math.min(memo[start][end -1], memo[start + 1][end]);
            start++;
            end++;
            if(end >= n){
                step++;
