...
public class Main {
   ...
   public static void main(String[] args) throws IOException {
        ...
        Stack<Pair> previous = new Stack<>();
        for(int i = 0; i < n; i++){
            int current = in.nextInt();

            while(!previous.empty() && current <= previous.peek().value){
                previous.pop();
            }

            out.append(!previous.empty() ? previous.peek().index + 1 : 0);
            out.append(" ");
            previous.add(new Pair(current, i));
        }
