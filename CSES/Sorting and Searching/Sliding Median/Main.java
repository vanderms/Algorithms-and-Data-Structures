...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i < k; i++){
            tree.update(numbers[i].order, 1);
        }
        int query = tree.query(1, pivot);

        out.append(onumbers[query].value);
        out.append(" ");

        for(int i = k; i < n; i++){
            tree.update(numbers[i - k].order, 0);
            tree.update(numbers[i].order, 1);
            query = tree.query(1, pivot);
            out.append(onumbers[query].value);
            out.append(" ");
        }
...
class SegmentTree {
    ...
    public int query(int node, int value){
        if(node >= this.size){
        ...
        int nodeR = nodeL + 1;
        if(arr[nodeL] >= value){
            return query(nodeL, value);
        }
        else return query(nodeR, value - arr[nodeL]);
    }
}
